...
object GitHubRepoMetadataDownloader extends Logger {
  ...
  val tempFolder = System.getProperty("java.io.tmpdir") + "/" + UUID.randomUUID() + "/"
  ...
  def addTask(task: String): String = {
    ...
    this.synchronized {
      try {
        if (!errorFlag) {
          val range = task.split("-")
          val noOfPartitions = (range(1).toInt - range(0).toInt) /
            KodeBeagleConfig.metaRepoRangeSize.toInt
          var nextStart=range(0).toInt
          for (i <- (1 to noOfPartitions)) {
            val subtask = nextStart + "-" + (nextStart + (KodeBeagleConfig.metaRepoRangeSize.toInt))

            if (!pendingTask.exists(p => p.equals(subtask.toString()))) {
              pendingTask.enqueue(subtask.toString())
            }
            nextStart = nextStart + KodeBeagleConfig.metaRepoRangeSize.toInt
          }
