...
class GitHubMetaDataHdfsCopier extends GitHubMetaDataRangeProcessor with Logger{
  ...
  protected val metadataDir: String = KodeBeagleConfig.metadataDir
  ...
  override def process(from: Int, to: Int): Unit = {
    ...
    try {
      ...
      val files = FileUtils.listFiles(new File(
            this.metadataDownloadTempDir), new WildcardFileFilter(
      ...
      for (file: File <- files){
        ...
        val destPath = new Path(this.metadataDir + file.getName())
